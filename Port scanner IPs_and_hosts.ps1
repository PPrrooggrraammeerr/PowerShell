...
$File_Path_IPs_and_hosts = "$env:USERPROFILE\Desktop\IPs and hosts.txt"
if (-not (Test-Path -Path $File_Path_IPs_and_hosts)) {
...
$File_Path_Communication_ports = "$env:USERPROFILE\Desktop\Communication ports.txt"
if (-not (Test-Path -Path $File_Path_Communication_ports)) {
New-Item -Path "$env:USERPROFILE\Desktop\Communication ports.txt" -ItemType File -Force -ErrorAction SilentlyContinue
} else {
Remove-Item -Path "$env:USERPROFILE\Desktop\Communication ports.txt" -Force -ErrorAction SilentlyContinue
New-Item -Path "$env:USERPROFILE\Desktop\Communication ports.txt" -ItemType File -Force -ErrorAction SilentlyContinue        
}
Start-Sleep -Seconds 1 -ErrorAction SilentlyContinue
Clear-Host -ErrorAction SilentlyContinue
